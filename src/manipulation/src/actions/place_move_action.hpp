...
public:
    explicit PlaceMoveAction(rclcpp::Node::SharedPtr node,
                             ...
                             double drop_height = 0.025,
                             double move_speed = 0.2)
        : node_(node),
          gripper_pub_(node_->create_publisher<std_msgs::msg::String>("/prongs/cmd", 10)),
          end_eff_pose_(end_eff_pose),
          linear_raise_action_(node, move_speed),
          linear_position_action_(node, move_speed),
          linear_drop_action_(node, move_speed),
          linear_reraise_action_(node, move_speed),
          linear_reset_action_(node),
